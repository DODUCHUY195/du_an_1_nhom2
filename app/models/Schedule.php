...
class Schedule extends BaseModel
{
    ...
    public function getSchedulesWithSeats(): array
    {
        $sql = "SELECT 
                    s.*, 
                    t.tour_name, 
                    t.tour_code,
                    (s.seats_total - s.seats_booked) AS available_seats
                FROM tour_schedule s
    ...
    public function increaseBookedSeats(int $scheduleId, int $count): bool
    {
        $sql = "UPDATE tour_schedule SET seats_booked = seats_booked + :count WHERE schedule_id = :schedule_id";
        $stmt = $this->db->prepare($sql);
        return $stmt->execute([
            ':count' => $count,
    ...
    public function decreaseBookedSeats(int $scheduleId, int $count): bool
    {
        $sql = "UPDATE tour_schedule SET seats_booked = GREATEST(seats_booked - :count, 0) WHERE schedule_id = :schedule_id";
        $stmt = $this->db->prepare($sql);
        return $stmt->execute([
            ':count' => $count,
            ':schedule_id' => $scheduleId,
